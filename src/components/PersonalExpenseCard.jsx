...
const PersonalExpenseCard = ({ fetchDataFromDB, data, index }) => {

  const deleteDocFromDB = async () => {
    const ans = prompt("Are you sure! Do ypu want to delete - type 'yes'");
    if (ans.toLowerCase() === 'yes') {
      db.collection('PersonalExpense').doc(data.expenseId).delete()
        .then(() => {
  ...
  return (
    <div key={index} className="min-h-full">
      <div className="flex  min-h-full border-t-4 border-red-500 md:flex justify-between gap-3 m-2 rounded-md shadow-md bg-white text-gray-700 px-4 py-2">
        <div className="w-full h-full">
          <div className="flex justify-between">
            <h1 className="w-min whitespace-nowrap p-2 text-red-500  bg-red-100 rounded-lg text-base font-semibold">
          ...
          <div className="mt-4 text-sm font-normal text-gray-700">
            ...
            <span className="whitespace-nowrap">{data.expenseDetails.title}</span> */}
            {data.expenseDetails.date} {data.expenseDetails.time && "| "+data.expenseDetails.time}
          </div>
          <div>
